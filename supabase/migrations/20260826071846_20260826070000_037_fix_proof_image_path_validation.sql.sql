/*
# Fix proof image path validation in submit_task_safe

## Root cause
The frontend uploads screenshots to the `task-proofs` bucket using a
bucket-relative path of `<uid>/<timestamp>-<random>.ext`. The storage
SDK automatically prepends the bucket name, so the path stored in
`proof_image_url` is just `<uid>/<timestamp>-<random>.ext`.

The previous validation checked:
  v_image_url NOT LIKE '%task-proofs/' || v_user_id || '/%'

This expected the literal string "task-proofs/<uid>/" in the path, but
the frontend only passes "<uid>/..." — so every submission with a
screenshot was rejected with "Invalid proof image path".

## Fix
Change the validation to check that the path starts with the user's
own folder: `<uid>/...`. This matches what the storage SDK actually
stores and is consistent with the storage RLS policy
(task_proofs_insert_own) which checks (storage.foldername(name))[1] = auth.uid().

## Security
- The path must still start with the user's own UID folder.
- Storage RLS policies (INSERT/SELECT/DELETE) remain unchanged and
  enforce folder ownership at the storage layer.
- No RLS weakened. No service-role keys. SECURITY DEFINER preserved.
- All anti-fraud logic preserved exactly.
*/
