...
-- Drop the existing function with DEFAULT param and recreate without it
-- The frontend always sends all 3 params, so DEFAULT is unnecessary
-- PostgREST has a known issue resolving functions with DEFAULT params in schema cache
...
-- Grant execute to authenticated (primary) and anon (for PostgREST schema cache discovery)
