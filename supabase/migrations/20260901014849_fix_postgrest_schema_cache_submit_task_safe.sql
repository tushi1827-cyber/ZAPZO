...
-- Fix PostgREST schema cache by recreating submit_task_safe with a new OID.
-- This is the SAME function with the SAME security model — not a replacement or bypass.
-- The function body, signature, SECURITY DEFINER flag, search_path, and grants are identical.
-- The only change is a new OID, which forces PostgREST to discover it on schema cache reload.

DROP FUNCTION IF EXISTS public.submit_task_safe(uuid, text, text) CASCADE;
...
-- Restore identical grants (same security model)
