...
-- Recreate submit_task_safe with a fresh OID to force PostgREST schema cache refresh.
-- The previous DROP/CREATE via migration didn't resolve the issue because PostgREST
-- may have cached the old OID. This migration drops and recreates the function,
-- then explicitly reloads the schema cache.

-- Drop the existing function
DROP FUNCTION IF EXISTS public.submit_task_safe(uuid, text, text) CASCADE;

-- Recreate with identical body
...
-- Restore grants (identical security model)
