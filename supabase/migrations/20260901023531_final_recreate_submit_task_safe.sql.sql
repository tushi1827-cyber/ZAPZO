...
-- Final attempt: DROP and recreate submit_task_safe with a completely fresh OID.
-- The previous recreation (OID 18019) is not being picked up by PostgREST
-- even though the schema cache IS reloading (review_withdrawal at OID 18043 IS visible).
-- This suggests the function's OID might be in a "dead zone" that PostgREST skips.
-- Let's try dropping and recreating to get a new OID.
