-- Drop and recreate submit_task_safe with a fresh OID to fix PostgREST visibility
-- PostgREST has a stale schema cache that doesn't pick up functions via CREATE OR REPLACE

DROP FUNCTION IF EXISTS public.submit_task_safe(uuid, text, text) CASCADE;

CREATE FUNCTION public.submit_task_safe(
  p_task_id uuid,
  p_proof_text text,
